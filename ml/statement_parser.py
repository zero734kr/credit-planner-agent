"""
Statement Parser — Module that automatically extracts transactions from PDF documents
...
Usage:
  ...
  transactions = parser.parse_pdf("/path/to/statement.pdf")
...
class StatementParser:
    ...
    def __init__(self):
        self.supported_formats = ["capital_one", "chase_credit", "chase_checking"]
    ...
    def parse_pdf(self, filepath: str) -> Dict:
        ...
        if fmt == "capital_one":
            ...
            raise ValueError(f"Unsupported statement format: {filepath}")
    ...
    def get_all_transactions(self, results: List[Dict]) -> List[Dict]:
        ...
        for r in results:
            ...
            for tx in r.get("transactions", []):
                tx_copy = dict(tx)
                tx_copy["card_name"] = card_name
                tx_copy["source"] = source
